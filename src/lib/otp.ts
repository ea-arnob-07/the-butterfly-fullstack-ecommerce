...
async function sendVerificationEmail(input: { email: string; name: string; code: string }) {
  ...
  if (!response.ok) {
    ...
    if (process.env.NODE_ENV !== 'production') {
      console.warn('Falling back to dev OTP because Resend failed.');
      return { delivered: false, developmentMode: true };
    }
...
async function sendPasswordResetEmail(input: { email: string; name: string; code: string }) {
  const apiKey = process.env.RESEND_API_KEY;
  const from = process.env.EMAIL_FROM || 'The Butterfly <onboarding@resend.dev>';

  if (!apiKey) {
    if (process.env.NODE_ENV !== 'production') {
      console.info(`[DEV OTP] Password Reset for ${input.email}: ${input.code}`);
      return { delivered: false, developmentMode: true };
    }
  ...
  const response = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from,
      to: [input.email],
      subject: `Reset your The Butterfly password`,
      html: `
        <div style="font-family:Arial,sans-serif;max-width:560px;margin:auto;padding:32px;color:#24151e">
          <div style="padding:28px;border:1px solid #f8c9dc;border-radius:24px;background:#fff8fb">
            <p style="margin:0 0 12px;color:#d4075a;font-size:12px;font-weight:700;letter-spacing:2px;text-transform:uppercase">The Butterfly · Your Dream Line</p>
            <h1 style="margin:0 0 12px;font-size:28px">Reset your password</h1>
            <p style="line-height:1.7;color:#6d5764">Hello ${escapeHtml(input.name)}, you requested to reset your password. Use the verification code below to set a new password.</p>
            <div style="margin:24px 0;padding:18px;border-radius:16px;background:#ffffff;text-align:center;font-size:34px;font-weight:800;letter-spacing:10px;color:#d4075a">${input.code}</div>
            <p style="line-height:1.7;color:#6d5764">This code expires in ${OTP_TTL_MINUTES} minutes. If you did not request this, please ignore this email.</p>
          </div>
        </div>`,
    }),
  });
...
export async function issuePasswordResetOtp(user: { id: string; email: string; name: string }, options?: { ignoreRateLimit?: boolean }) {
  ...
  try {
    const delivery = await sendPasswordResetEmail({ email: user.email, name: user.name, code });
    return {
      sent: true,
      retryAfter: OTP_RESEND_SECONDS,
      expiresAt,
